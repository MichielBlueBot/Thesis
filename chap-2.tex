...
In this chapter I will explain the cox proportional hazards model. This is a model that is used in survival analysis. The first section will explain what survival analysis means and how it is different from the generalized linear models. Next I will define what the proportional hazards assumption is and lastly I will show kaplan-meier survival curves and how we can compute them from hazard functions.
...
\end{split}
\end{equation}
It is directly given by the exponential of the co\"effici\"ent for the explanatory variable. This reflects the proportional hazards assumption that hazard ratios do not depend on time and that explanatory variables have an exponential effect on the hazard.
...
Kaplan-Meier survival curves are often used in survival analysis. These curves show the probability of surviving up to any point t. It is thus a declining curve starting from 1 at $t=0$. An example curve is shown on figure \ref{fig:cox-example-kaplan-meier}.
\begin{figure}
	\centering
...
\end{split}
\end{equation}
This result makes sense, let me explain it in a more intuitive way. Let's assume the event we are talking about is dying. The hazard function then tells me what the risk is of me dying at time $t$. Equation \ref{eq:cox-hazard} tells me that this is equal to $-\frac{S'(t)}{S(t)}$. There are three parts to explain here:
\begin{itemize}
	\item $\frac{1}{S(t)}$: The first thing I have to do is survive up to time $t$. Otherwise it makes no sense to ask what the risk of dying is at time $t$. The survival function tells me how probable it is to survive up to time $t$ so dividing by this quantity exactly gives me what I need. If $S(t)$ is large, then this means many people survive up to time $t$ and so the risk is low. If $S(t)$ is small then few people actually survive up to time $t$ and thus the risk of having to survive up to this point gets bigger.
	\item $S'(t)$: Now that we have survived up to time $t$, we need to calculate the risk of dying at this point in time. Remember that the survival curve declines whenever people die at certain points in time. The slope of this function therefore tells me the risk of dying. If $S'(t)$ is very large, it means that the survival curve declines rapidly at this point in time, and thus many people die here. The risk of dying thus becomes large.
	\item The hazard is supposed to be a positive quantity. Since the survival function is a strictly declining curve (people usually don't resurrect), $S'(t)$ is negative. Therefore we need the minus sign to end up with a positive result.
\end{itemize}
Next we can observe that
\begin{equation}
\begin{split}
\label{eq:cox-survival}
\frac{\partial log(S(t))}{\partial t} = \frac{S'(t)}{S(t)} = -\lambda(t)
\end{split}
\end{equation}
