...
In this chapter we will explain the concepts of generalized linear models~\cite{caltechmachinelearning}\cite{wikiglm}. This term indicates a generalization of simple linear regression that allows for a wide range of output variables. First we will define exactly what we mean by a predictive model (\ref{sec:glm-predictive-models}). Then we will go over the basics of linear models, gradually building up to the definition of generalized linear models (\ref{sec:glm-classicallinearmodels}). Next, we will describe what actual data looks like and how a model is trained from it (\ref{sec:glm-trainingamodel}). After that we will tackle the more recent innovation of regularization that will greatly improve our previous models by exploiting the bias-variance trade-off to reduce overfitting (\ref{sec:glm-regularization}). Lastly we will outline the validation method that will be used to test the performance of the models (\ref{sec:glm-validation}).
